  ...
  try {
    ...
    if (self.options.idpCert) {
      try {
        ...
        if (!((doc && doc.documentElement) && self.validateSignature(xml, doc.documentElement))) {
    ...
    var assertion = true; //xmlCrypto.xpath(xmlDomDoc, "//*[local-name(.)='Assertion']");
    // console.log('assertion', assertion);
    // console.log('xmlDomDoc', xmlDomDoc);
    if (assertion) {
      profile = {};
    ...
    Accounts.saml.debugLog('saml_utils.js', '358', 'Unknown SAML response message.. Error: ' + error + error.stack, true);

    return callback(new Error('Unknown SAML response message' + error + error.stack), null);
  }
};
...
// No longer used, since MSFT does not return encrypted XML
